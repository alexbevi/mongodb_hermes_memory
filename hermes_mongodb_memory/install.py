"""Installer CLI for the MongoDB Hermes memory provider."""
...
def _load_direct_paths_module() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("hermes_mongodb_memory._paths", paths_file)
...
def plugin_source_dir() -> Path:
    """Return the installed provider package directory."""
    return Path(__file__).resolve().parent


def plugin_target_dir(hermes_home_path: str | Path | None = None) -> Path:
    """Return the Hermes memory plugin destination for MongoDB."""
...
def install_plugin(
    *,
    ...
    """Install the provider into Hermes' user plugin directory."""
...
def uninstall_plugin(*, hermes_home_path: str | Path | None = None) -> Path:
    """Remove the provider from Hermes' user plugin directory."""
    target = plugin_target_dir(hermes_home_path)
    if target.exists():
        shutil.rmtree(target)
    return target
...
def is_installed(*, hermes_home_path: str | Path | None = None) -> bool:
    """Return whether the provider is installed for Hermes discovery."""
    target = plugin_target_dir(hermes_home_path)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="hermes-mongodb-memory",
        description="Install the MongoDB memory provider for Hermes Agent.",
    )
    parser.add_argument(
        "--hermes-home",
        help=(
    ...
    install = subparsers.add_parser(
        "install",
        help="Install MongoDB into Hermes' memory provider plugin directory.",
    )
    install.add_argument(
        "--force",
        action="store_true",
        help="Replace an existing MongoDB plugin directory.",
    )

    subparsers.add_parser(
        "uninstall",
        help="Remove MongoDB from Hermes' memory provider plugin directory.",
    )
    subparsers.add_parser(
        "status",
        help="Show whether MongoDB is installed for Hermes memory discovery.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if command == "install":
            ...
            print(f"Installed MongoDB Hermes provider to {target}")
            print("Next steps:")
            print("  hermes config set memory.provider mongodb")
            print("  hermes memory setup")
            print("  hermes mongodb init-indexes")
            return 0

        if command == "uninstall":
            target = uninstall_plugin(hermes_home_path=args.hermes_home)
            print(f"Removed MongoDB Hermes provider from {target}")
            return 0

        if command == "status":
            target = plugin_target_dir(args.hermes_home)
            if is_installed(hermes_home_path=args.hermes_home):
                print(f"MongoDB Hermes provider is installed at {target}")
                return 0
            print(f"MongoDB Hermes provider is not installed at {target}")
            return 1
